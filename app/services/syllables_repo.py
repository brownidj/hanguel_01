...
"""Syllable repository helpers (service layer).
Responsibilities:
...
The public functions mirror the legacy helpers previously defined in `main.py` so
we can delegate from the UI layer without behaviour change.

Expected YAML location:
- data/syllables.yaml

Expected YAML shape (flexible):
- Either a mapping keyed by block type names (e.g., "A_RightBranch") to a list of syllables
...
def _project_root() -> Path:
    ...
    Assumes this file lives at: <root>/app/services/syllables_repo.py
